...
const BarberCard = ({ barber, index = 0 }: { barber: Barber; index?: number }) => {
  return (
    <Link
      ...
      className="group relative block overflow-hidden rounded-sm bg-[#111] border border-[#DD873C]/10 hover:border-[#DD873C]/40 transition-all duration-500 hover:shadow-[0_0_30px_hsl(32_70%_55%/0.15)] animate-fade-up"
    ...
    >
      ...
      <div className="relative aspect-[3/4] overflow-hidden bg-[#111]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5">
          <p className="text-[10px] uppercase tracking-[0.2em] text-[#DD873C] mb-1">{barber.chair}</p>
          <h3 className="display text-3xl text-[#F7F4EF] leading-none">{barber.name}</h3>
        </div>
      </div>

      {/* Footer card — sin estrellas */}
      <div className="flex items-center justify-end px-4 py-3 border-t border-[#DD873C]/10">
        <span className="text-[11px] uppercase tracking-[0.2em] text-[#DD873C] group-hover:translate-x-1 transition-transform">
          Ver perfil →
        </span>
      </div>
    </Link>
